...
public interface ServicoPendenteVIEWRepository
	...
	@Query(value = "SELECT * FROM view_servico_pendente WHERE id_funcionario = ?1 AND month(data_hora) = ?2 AND year(data_hora) = ?3 AND finalizado = 0", nativeQuery=true)
	public List<ServicoPendenteVIEW> pegerServicosPendentesFuncionario(Long idFuncionario, Integer mes, Integer ano);
	
	@Query(value = "SELECT * FROM view_servico_pendente WHERE id_funcionario = ?1 AND month(data_hora) = ?2 AND year(data_hora) = ?3 AND day(data_hora) = ?4 AND finalizado = 0", nativeQuery=true)
	...
	@Query(value = "SELECT * FROM view_servico_pendente WHERE id_estabelecimento = ?1 AND month(data_hora) = ?2 AND year(data_hora) = ?3 AND day(data_hora) <> ?4 AND finalizado = 0", nativeQuery=true)
